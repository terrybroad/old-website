...
(function(global) {
  ...
  // VolofVol is my addition to store the value of all of the values
  // travelling down each edge (connection) in the netwo
  // It is probably horrendously inefficient but it is just a hack
  var VolofVol = function(sx,sy,depth,v_sx,v_sy,v_depth) //need to add functionality for JSON in_out, maybe?
  {
      this.sx = sx;
      this.sy = sy;
      this.depth = depth;
      ...
      for(var i=0;i<n;i++) 
      {
        this.vols.push(new Vol(this.v_sx, this.v_sy, this.v_depth, 0.0));
      }
  }
  ...
  VolofVol.prototype = 
  {
    ...
    getVolsLength: function(){
      return this.vols.length;
    },
    getVol: function(x, y, d) { 
      var ix=((this.sx * y)+x)*this.depth+d;
      return this.vols[ix];
    },
    getVal: function(x, y, d, vx, vy, vd) { 
      var ix=((this.sx * y)+x)*this.depth+d;
      var val;
      if(this.vols[ix] != undefined) {val = this.vols[ix].get(vx,vy,vd);}
        else {val = 0;}
      return val; 
    },
    setVal: function(x, y, d, vx, vy, vd, vv) { 
      var ix=((this.sx * y)+x)*this.depth+d;
      this.vols[ix].set(vx,vy,vd,vv); 
    },
    getSx: function() {
      return this.sx;
    },
  ...
  global.Vol = Vol;
  global.VolofVol = VolofVol;
})(convnetjs);
(function(global) {
  "use strict";
  var Vol = global.Vol; // convenience
  var VolofVol = global.VolofVol;
...
(function(global) {
  ...
  var ConvLayer = function(opt) {
    ...
    this.A = new Vol(this.out_sx |1, this.out_sy |1, this.out_depth |1, 0.0);
    ...
    this.VolofVolofActivations = new VolofVol(this.out_sx-1 |1,this.out_sy-1 |1,this.out_depth |1,this.sx |1,this.sy |1,this.in_depth |1);
  ...
  ConvLayer.prototype = {
    forward: function(V, is_training) {
      ...
      var vva = this.VolofVolofActivations;
              ...
              for(var fx=0;fx<f.sx;fx++) {
                ...
                if(oy>=0 && oy<V_sy && ox>=0 && ox<V_sx) {
                  for(var fd=0;fd<f.depth;fd++) {
                    ...
                    //var index = ((this.VolofVolofActivations.sx*ay)+ax)*this.VolofVolofActivations.depth+d;
                    //if(vva.getVolsLength() > index)
                    //{
                    //this.VolofVolofActivations.vols[((this.VolofVolofActivations.sx*ay)+ax)*this.VolofVolofActivations.depth+d].w[((f.sx * fy)+fx)*f.depth+fd] = activation; 
                    if(ay+1 < this.out_sy && ax+1 < this.out_sx)
                    {
                      this.VolofVolofActivations.setVal(ax,ay,d,fx,fy,fd,activation);
                    }
                    //}
                    //else{console.log("cunt");}
                    a += activation;
                  }
                }
              }
  ...
  var FullyConnLayer = function(opt) {
    ...
    this.out_sy = 1;
    this.layer_type = 'fc';
    this.A = new Vol(this.out_sx |0, this.out_sy |0, this.out_depth |0, 0.0);
    ...
    this.VolofVolofActivations = new VolofVol(1,1, this.out_depth ,1,1,this.num_inputs);
  }
  ...
  FullyConnLayer.prototype = {
    forward: function(V, is_training) {
      this.in_act = V;
      this.A = new Vol(1, 1, this.out_depth, 0.0);
      var Vw = V.w;
      for(var i=0;i<this.out_depth;i++) {
        var a = 0.0;
        var activation = 0.0;
        ...
        for(var d=0;d<this.num_inputs;d++) {
          activation = Vw[d] * wi[d];
          a += activation; // for efficiency use Vols directly for now
          this.VolofVolofActivations.setVal(0,0,i,0,0,d,activation);
        }
        a += this.biases.w[i];
        this.A.w[i] = a;
      }
...
(function(global) {
  ...
  PoolLayer.prototype = {
    forward: function(V, is_training) {
      ...
      this.A = new Vol(this.out_sx, this.out_sy, this.out_depth, 0.0);
      ...
      for(var d=0;d<this.out_depth;d++) {
        ...
        for(var ax=0; ax<this.out_sx; x+=this.stride,ax++) {
          ...
          for(var ay=0; ay<this.out_sy; y+=this.stride,ay++) {
            ...
            n = ((this.out_sx*ay)+ax)*this.out_depth+d;
            this.switchx[n] = winx;
            this.switchy[n] = winy;
            //n++;
            this.A.set(ax, ay, d, a);
...
(function(global) {
  ...
  var InputLayer = function(opt) {
    ...
    // required: depth
    this.out_depth = getopt(opt, ['out_depth', 'depth'], 1);
